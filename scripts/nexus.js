const nexusStyleExcludedCode=[
    'code','pre','samp','pre *','kbd','tt',
    'datalist','progress','meter','var'
].join(', ');

function nexusGetShadowColor(node){
    ...
    const parseColor=colorStr=>{
        ...
        if(colorStr.startsWith('rgba')){
            ...
            return [parseInt(match[1]),parseInt(match[2]),parseInt(match[3]),parseFloat(match[4])*255];
...
function nexusApplyStyle(node){
    if(node.matches(nexusStyleExcludedCode)){
        node.style.fontFamily='MonoLisa Commilitia, CommilitiaFont UI SC';
        node.style.webkitTextStroke='0.08px';
        node.style.textShadow=`0.45px 0.45px 0 ${nexusGetShadowColor(node)}`;
        return; 
    }
    node.style.fontFamily='CommilitiaFont UI SC';
    node.style.webkitTextStroke='0.08px';
    node.style.textShadow=`0.45px 0.45px 0 ${nexusGetShadowColor(node)}`;
}

function nexusRecursiveExecute(node,selector,func){
    func(node);
    node.querySelectorAll(selector).forEach(child=>func(child));
}

function nexusSelectInScope(node,selector){
...
document.addEventListener('DOMContentLoaded',async()=>{
    ...
    document.documentElement.style.setProperty('--entry-content-text-color','#000000');
    document.querySelectorAll('*').forEach(node=>{
        if(!(node instanceof HTMLElement))return;
        nexusApplyStyle(node);
    });
    new MutationObserver(mutations=>{
        mutations.forEach(mutation=>{
            mutation.addedNodes.forEach(node=>{
                if(!(node instanceof HTMLElement))return;
                nexusRecursiveExecute(node,'*',nexusApplyStyle);
                let nodeX;
                nodeX=nexusSelectInScope(node,'.post-footer div p');
                if(nodeX)nodeX.innerHTML=nodeX.innerHTML.replace(/Q\.E\.D\./g,'Quad erat demonstrandum.');
